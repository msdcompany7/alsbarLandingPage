...
const contactNavItem = siteConfig.nav[siteConfig.nav.length - 1];
const mainNavItems = siteConfig.nav.slice(0, -1);
...
function SearchNavButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "inline-flex h-10 w-10 items-center justify-center rounded-xl text-text-secondary transition-colors hover:bg-surface-alt hover:text-primary",
        className,
      )}
    >
...
export function SiteHeader({ phone, phoneHref, categories }: SiteHeaderProps) {
            ...
            <ul className="hidden items-center justify-center gap-1 lg:flex xl:gap-2">
              ...
              <li className="flex items-center gap-0.5">
                <Link
                  ...
                  aria-current={isActiveNav(contactNavItem.href, pathname) ? "page" : undefined}
                  className={cn(
                    "relative rounded-xl px-5 py-3 text-base font-semibold tracking-tight transition-colors sm:px-6 sm:text-[17px] lg:py-3.5 lg:text-lg",
                    isActiveNav(contactNavItem.href, pathname)
                      ? "text-primary"
                      : "text-text-secondary hover:bg-surface-alt hover:text-primary",
                  )}
                >
                ...
                <SearchNavButton onClick={openSearch} className="lg:h-11 lg:w-11" />
              </li>
            </ul>

            <div className="flex shrink-0 items-center justify-end gap-2 sm:gap-3">
              <SearchNavButton onClick={openSearch} className="lg:hidden" />
        ...
        >
          <Container className="flex max-h-[calc(100dvh-5.25rem)] flex-col gap-1 overflow-y-auto py-4">
            ...
            <button
              type="button"
              onClick={openSearch}
              className="flex items-center gap-3 rounded-xl px-4 py-4 text-lg font-semibold text-text-primary transition-colors hover:bg-surface-alt"
            >
